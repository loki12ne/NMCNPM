...
// Ensure DB schema compatibility (idempotent)
async function ensureSchema() {
  try {
    await client.query(`
      ALTER TABLE Tasks 
      ADD COLUMN IF NOT EXISTS start_time TIME NOT NULL DEFAULT '00:00',
      ADD COLUMN IF NOT EXISTS end_time TIME NOT NULL DEFAULT '01:00';
    `);
    await client.query(`
      ALTER TABLE Tasks 
      ALTER COLUMN time SET DEFAULT 60;
    `);
  } catch (e) {
    console.error('Schema check/update failed:', e.message);
  }
}
...
router.get('/plans', isAuthenticated, async (req, res) => {
  try {
    ...
    // Get classes
    const classesQuery = `
      SELECT id, class_name as title, subject, start_date::text as date,
    ...
    // Get tasks
    const tasksQuery = `
      SELECT id, title, description, subject, time, due_date::text as due_date, start_time, end_time,
             created_at, due_date::text as date
      FROM Tasks 
      WHERE username = $1 
      ORDER BY due_date
    `;
    ...
    // Get exams
    const examsQuery = `
      SELECT id, subject, type as exam_type, exam_date::text as date,
...
router.post('/classes', isAuthenticated, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: error.detail || error.message || 'Internal server error' });
  }
});
