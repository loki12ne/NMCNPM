  ...
  try {
    ...
    await client.query(`
      INSERT INTO Questions (username, text_content, subject, date_posted, is_answered) VALUES
        ...
        ('learner', 'Phương trình phản ứng trung hòa giữa axit và bazơ', 'Chemistry', '2025-05-08 13:20:00', FALSE),
        ('nguyenvana', 'Giải hệ phương trình: 2x + 3y = 7, x - y = 1', 'Math', '2025-05-12 11:40:00', FALSE),
        ('tranvanb', 'Định luật bảo toàn khối lượng trong hóa học', 'Chemistry', '2025-05-18 15:30:00', FALSE),
        ('learner', 'Tính giới hạn: lim(x→0) sin(x)/x', 
        'Math', '2025-05-22 14:50:00', FALSE)
    ...
    await client.query(`
      INSERT INTO Answers (question_id, user_ask, user_answer, text_content, date_posted) VALUES
        ...
        (4, 'nguyenvana', 'teacher1', 'Định luật Ohm: U = I × R, trong đó U là hiệu điện thế, I là cường độ dòng điện, R là điện trở', '2025-03-06 09:00:00'),
        (5, 'tranvanb', 'tutor', 'y'' = 3x^2 + 4x - 5', '2025-03-13 10:30:00')
      ON CONFLICT (answer_id) DO NOTHING;
    `);

    await client.query(`
      INSERT INTO FeedBacks (question_id, username, rating, comment, date_posted) VALUES
        (1, 'learner', 4, 'Clear explanation!', '2025-06-04 09:00:00'),
        (2, 'learner', 5, 'Very good!', '2025-06-04 10:00:00'),
        (4, 'nguyenvana', 5, 'Accurate formula, thanks teacher!', '2025-03-07 10:30:00'),
        (5, 'tranvanb', 4, 'Correct derivative, easy to understand', '2025-03-14 11:45:00')
    ...
    await client.query(`
      INSERT INTO TutorPerformance (username, average_rating, questions_answered, total_feedback, last_updated) VALUES
        ('tutor', 4.00, 2, 2, CURRENT_TIMESTAMP),
        ('teacher1', 5.00, 2, 2, CURRENT_TIMESTAMP)
      ON CONFLICT (username) DO NOTHING;
    `);
